...
class ArtistCoordinator: BaseCoordinator {
    ...
    init(artistId: String, factory: ArtistFlow, router: Router, serviceManager: ServiceManagerProtocol, coordinatorFactory: CoordinatorFactoryProtocol) {
...
private extension ArtistCoordinator {
    func runArtistModule() {
        let (artistModule, presenter) = factory.makeArtistModule(with: artistId, serviceManager: serviceManager)
